---
title: "クイックスタート"
description: "JARFISをインストールし、11の専門Agentと構造化されたPhaseで最初のAI駆動ソフトウェア開発ワークフローを実行しましょう。"
...
AIとともにソフトウェアを出荷しましょう — スラッシュコマンド1つ、11の専門Agent、Human Gateで構成された構造化されたPhase。
...
| 3 | UX Design | 画面設計、SVG asset、デザイントークン（条件付き — UIが必要な場合のみ） |
...
JARFISは11の専門Agentを調整しており、各Agentは現在のPhaseとプロジェクト要件に関係する場合にのみ活性化されます:
...
| **UX Designer** | 画面設計、インタラクション設計、SVG asset作成、デザイントークン |
...
| **Advocate** | 提案されたソリューションの利点を擁護、トレードオフの防衛 |
| **Critic** | 前提への疑問提起、リスクと弱点の特定 |
...
- [API Reference](/ja/docs/api-reference)ですべてのcommandと設定を確認する
